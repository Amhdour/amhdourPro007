...
export function generateStaticParams() {
  return dictionaries.de.caseStudies.studies.map((s) => ({ slug: s.slug }));
}

export async function generateMetadata({ params }: CaseStudyPageProps): Promise<Metadata> {
  const { slug } = await params;
  const study = dictionaries.de.caseStudies.studies.find((s) => s.slug === slug);
  if (!study) return { title: "Nicht gefunden" };
  return { title: `${study.title} | Ahmed Amhdour`, description: study.overview };
}

export default async function GermanCaseStudyPage({ params }: CaseStudyPageProps) {
  const { slug } = await params;
  const t = dictionaries.de.caseStudies;
    ...
    <div className="text-site-text bg-site-bg">
      ...
      <main className="min-h-screen pt-[75px]">
        <article className="max-w-[800px] mx-auto px-4 py-16">
          <Link href="/de#portfolio" className="text-site-primary hover:underline text-sm mb-8 inline-block">{t.backLink}</Link>
