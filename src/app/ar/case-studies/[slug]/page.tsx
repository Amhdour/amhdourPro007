...
export function generateStaticParams() {
  return dictionaries.ar.caseStudies.studies.map((s) => ({ slug: s.slug }));
}

export async function generateMetadata({ params }: CaseStudyPageProps): Promise<Metadata> {
  const { slug } = await params;
  const study = dictionaries.ar.caseStudies.studies.find((s) => s.slug === slug);
  if (!study) return { title: "غير موجود" };
  return { title: `${study.title} | أحمد امحدور`, description: study.overview };
}

export default async function ArabicCaseStudyPage({ params }: CaseStudyPageProps) {
  const { slug } = await params;
  const t = dictionaries.ar.caseStudies;
    ...
    <div className="text-site-text bg-site-bg">
      ...
      <main className="min-h-screen pt-[75px]">
        <article className="max-w-[800px] mx-auto px-4 py-16">
          <Link href="/ar#portfolio" className="text-site-primary hover:underline text-sm mb-8 inline-block">
            {t.backLink}
          </Link>
          ...
          <section className="mb-10">
            ...
            <ul className="list-disc pr-6 space-y-2">
              {study.approach.map((step, i) => (
                <li key={i} className="text-site-text/80">{step}</li>
              ))}
            </ul>
          </section>
          <section className="mb-10">
            ...
            <div className="flex flex-wrap gap-2">
              {study.tools.map((tool) => (
                <span key={tool} className="px-3 py-1 rounded-full text-sm bg-site-primary/10 text-site-primary font-medium">{tool}</span>
              ))}
            </div>
          </section>
          <section className="mb-10">
